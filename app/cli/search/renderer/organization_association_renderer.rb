...
module CLI
  class Search
    class Renderer
      class OrganizationAssociationRenderer
        include Formatter
        ...
        def render
          puts format_separator("Users")
          data[:associations][:users].each do |organization|
            output = TTY::Table.new(header: [format_header("User Fields"), format_header("Values")])
